...
export function MissionsSidebar({ missions, missionStates, onSelect, onClose }: MissionsSidebarProps) {
  return (
    <div className="fixed top-0 right-0 w-80 h-full bg-bg-secondary border-l-2 border-border z-[900] flex flex-col shadow-[-10px_0_30px_rgba(0,0,0,0.3)] sidebar-active">
      <div className="flex justify-between items-center px-4 py-4 border-b border-border">
        <h3 className="text-accent font-bold flex items-center gap-2"><ClipboardList className="w-4 h-4" /> Misiones Disponibles</h3>
        <button
          onClick={onClose}
        ...
        {missions.map((mission, idx) => {
          ...
          return (
            ...
            >
              ...
              <p className="text-text-dim text-xs">{mission.subtitle}</p>
              <span className={`inline-flex items-center gap-1 mt-1.5 text-[0.7rem] px-2 py-0.5 rounded-full ${
                ...
                {isCompleted && <><CheckCircle className="w-3 h-3" /> Completada</>}
                {state === 'current' && <><Circle className="w-3 h-3" /> Disponible</>}
                {isLocked && <><Lock className="w-3 h-3" /> Bloqueada</>}
              </span>
            </div>
          );
        })}
